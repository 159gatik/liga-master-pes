...
export default function Navbar() {
    ...
    // Links base que ven todos (Agregamos Despachos aquí)
    const links = [
    ...
    // Lógica dinámica para Fichajes / Inscripción
    if (user) {
        links.push({ name: 'Fichajes', href: '/fichajes' });

        // Si es Admin, le agregamos la oficina de control
        if (isAdmin) {
            links.push({ name: 'Admin', href: '/admin' });
        } else {
            // Si es un DT, le agregamos link a su perfil
    ...
    return (
        <nav className="bg-[#111111] border-b-2 border-[#c9a84c] px-6 md:px-10 sticky top-0 z-50 shadow-2xl font-barlow-condensed">
            <div className="max-w-7xl mx-auto flex items-center justify-between">

                {/* LOGO */}
                <div className="flex items-center gap-8">
                    <Link href="/" className="font-bebas text-2xl md:text-3xl text-[#c9a84c] tracking-[4px] py-4 hover:opacity-80 transition-opacity italic">
                        EL LEGADO
                    </Link>
                        ...
                        {links.map((link) => {
                            ...
                            return (
                                <Link
                                    key={link.name}
                                    href={link.href}
                                    className={`font-semibold text-[11px] md:text-[13px] tracking-[2px] uppercase px-4 md:px-5 py-6 border-b-4 transition-all duration-200 ${isActive
                ...
                <div className="flex items-center gap-6">
                    {user ? (
                        ...
                        <div className="flex items-center gap-5">
                            <Link
                                href="/login"
                                    className="text-[13px] font-bold text-[#888] hover:text-white transition-colors tracking-[2px] uppercase"
                            >
                                Login
                            </Link>
                            <Link
                                href="/register"
                                    className="bg-[#c9a84c] text-black px-4 py-2 font-bebas text-lg tracking-widest hover:bg-white transition-all shadow-md italic"
                            >
                                Inscribirse
                            </Link>
                        </div>
